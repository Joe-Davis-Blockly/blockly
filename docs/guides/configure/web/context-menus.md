...
![The default context menu for a block][block-default-menu-image]{: width="200"}
...
components](#show_a_context_menu_on_a_custom_object). Blockly provides standard
...
1.  If the component is a workspace or block, checks to see if the specific
    ...
    customizing the context menu](#customize_per_block). If so, it passes the
...
yourself](/blockly/guides/configure/web/workspace_comment#enable_workspace_comments).
...
| ------------ | ---------------------------------- | --------------------------------------------------- |
| `'enabled'`  | Shows that the item is active.     | ![An enabled option][enabled-image]{: width="150"}  |
| `'disabled'` | Shows that the item is not active. | ![A disabled option][disabled-image]{: width="150"} |
...
## Customize context menus per block type or workspace {:#customize_per_block}

After Blockly has [generated an array of context menu
items](#how_context_menus_work), you can customize it for individual blocks or
...
1.  [Add templates](#add_a_template) to the registry for your context menu
    items.
...
[ContextMenuOption]: /com/blockly/reference/js/blockly.contextmenuregistry_namespace.contextmenuoption_typealias
[LegacyContextMenuOption]: /blockly/reference/js/blockly.contextmenuregistry_namespace.legacycontextmenuoption_interface
[coordinate-systems]: /blockly/guides/configure/web/metrics_manager#coordinate_systems
