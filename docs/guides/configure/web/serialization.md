...
[Serializers](/blockly/guides/configure/web/serialization#serializer_hooks)
...
3.  **Extra state is deserialized.** See the
    [Mutators](/blockly/guides/create-custom-blocks/mutators) documentation for
    more info.
4.  **The block is connected to its parent (if one exists).**
    5.  **Icons are deserialized.** Individual icons are deserialized in an
        *arbitrary order*.
    6.  **Fields are deserialized.** Individual fields are deserialized in an
        *arbitrary order*.
    7.  **Input blocks are deserialized.** This includes blocks connected to
        value inputs and statement inputs. Individual inputs are deserialized in
        an *arbitrary order*.
    8.  **Next blocks are deserialized.**
...
[validator](/blockly/guides/create-custom-blocks/fields/validators#registering_a_local_validator).
...
*   A `priority`, which is used to determine the [deserialization
    order](/blockly/guides/configure/web/serialization#deserialization_order).
