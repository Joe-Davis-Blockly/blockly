...
5.  **Icons are deserialized.** Individual icons are deserialized in an
        *arbitrary order*.
    1.  **Fields are deserialized.** Individual fields are deserialized in an
        *arbitrary order*.
    2.  **Input blocks are deserialized.** This includes blocks connected to
        value inputs and statement inputs. Individual inputs are deserialized in
        an *arbitrary order*.
    3.  **Next blocks are deserialized.**
